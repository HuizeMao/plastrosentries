...
##deeper layer ends
    ...
    model = Model(inputs = img_input, outputs = out, name='G1')
    
    return model


model= FCN(200,200)
#model = load_model("G1.h5")
 ##Config    
model.compile(loss='categorical_crossentropy',
            optimizer = keras.optimizers.Adam(lr=0.03),
            metrics=['acc',MeanIoU(num_classes=6)])
##save best
mc = ModelCheckpoint('G1_1.h5', monitor='val_loss', mode='max', verbose=1, save_best_only=True)

X_train=X_train/255.0
history = model.fit(X_train, Y_train, batch_size = 32, epochs = 10,verbose = 1, validation_split=0.15, shuffle=True,callbacks=[mc])
                
##save
model.save('G1_2.h5') #callbacks automatically saves
