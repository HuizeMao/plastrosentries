...
def FCN(input_height,input_width):
    ...
    conv1=conv(64*2*2,(3,3),"same",img_input)
    #conv1 = Dropout(0.2)(conv1)
    conv1=conv(64*2*2,(3,3),"same",conv1)
    pool1 = MaxPooling2D((2, 2))(conv1)
     
    conv2=conv(128*2*2,(3, 3),'same',pool1)
    #conv2 = Dropout(0.2)(conv2)
    conv2=conv(128*2*2,(3, 3),'same',conv2)
    pool2 = MaxPooling2D((2, 2))(conv2)

    conv3 = conv(256*2*2,(3, 3),'same',pool2)
    #conv3 = Dropout(0.2)(conv3)
    conv3 = conv(256*2*2,(3, 3),'same',conv3)
    
##deeper layer begins 
    #add conv4 with size(25) do  not downsample again
    pool3 = MaxPooling2D((2, 2))(conv3)

    conv33 = conv(512*2*2,(3,3),'same',pool3)
    #conv33 = Dropout(0.2)(conv33)
    conv33 = conv(512*2*2,(3,3),'same',conv33)

    
    #update up1 to concat (upsample conv4) with conv3 
    up0 = concatenate([UpSampling2D((2, 2))(conv33), conv3], axis=-1)
    conv33D = conv(256*2*2,(3, 3),'same',up0)
    #conv33D = Dropout(0.2)(conv33D)
    conv33D = conv(256*2*2,(3, 3),'same',conv33D)
##deeper layer ends

    ##Upsampling (Decoder)
    up1 = concatenate([UpSampling2D((2, 2))(conv33D), conv2], axis=-1)
    conv4 = conv(128*2*2,(3, 3),'same',up1)
    #conv4 = Dropout(0.2)(conv4)
    conv4 = conv(128*2*2,(3, 3),'same',conv4)

    up2 = concatenate([UpSampling2D((2, 2))(conv4), conv1], axis=-1)
    conv5 = conv(64*2*2,(3, 3),'same',up2)
    #conv5 = Dropout(0.2)(conv5)
    conv(64*2*2,(3, 3),'same',conv5)
...
model= load_model("bigfeature.h5")
 ##Config    
model.compile(loss='categorical_crossentropy',
            optimizer = keras.optimizers.Adam(lr=0.03),
            metrics=['acc',MeanIoU(num_classes=6)])
##save best
mc = ModelCheckpoint('big.h5', monitor='val_loss', mode='max', verbose=1, save_best_only=True)
...
for _ in range(4):
    for i in range(7):
        ...
        history = model.fit(X_train, Y_train, batch_size = 5, epochs = 1,verbose = 1, validation_split=0.15, shuffle=True,callbacks=[mc])
        ...
        del X_train, Y_train
    model.save('bigsfeature.h5') #callbacks automatically saves
##save
model.save('bigfeature.h5') #callbacks automatically saves
